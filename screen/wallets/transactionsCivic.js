...
const WalletTransactionsCivic = ({ navigation }) => {
  const {
    wallets,
    saveToDisk,
    setSelectedWalletID,
  ...
  useEffect(() => {
    ...
    setOptions({
      headerStyle: {
        backgroundColor: '#FCECCA',////actual header color
        borderBottomWidth: 0,
        //elevation: 0,
        // shadowRadius: 0,
  ...
  useEffect(() => {
    setOptions({ statusBarStyle: 'light', 
      //barTintColor: WalletGradient.headerColorFor(wallet.type)  ///not effectin ganything
    });
...
export default WalletTransactionsCivic;

WalletTransactionsCivic.navigationOptions = navigationStyle({}, (options, { theme, navigation, route }) => {
  return {
    headerRight: () => (
      <TouchableOpacity
        accessibilityRole="button"
...
WalletTransactionsCivic.propTypes = {
