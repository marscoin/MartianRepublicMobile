...
class Torsbee {
  ...
  async post(path, options) {
    ...
    const response = {};
    try {
      const torResponse = await tor.post(
        uri,
      ...
      (async () => {
        ...
        try {
          if (!this.constructor._testConn) {
            ...
            const target =
              "explorerzydxu5ecjrkwceayqybizmpjjznk5izmitf2modhcusuqlid.onion:110";
            this.constructor._testConn = await tor.createTcpConnection(
              { target },
              (data, err) => {
                if (err) {
                ...
                if (!json || typeof json.result === "undefined")
                  return this.constructor._rejectReference(
                    new Error(
                      "Unexpected response from TOR socket: " +
                        JSON.stringify(json)
                    )
                  );
...
module.exports.testHttp = async () => {
  const api = new Torsbee({
    baseURI:
      "http://explorerzydxu5ecjrkwceayqybizmpjjznk5izmitf2modhcusuqlid.onion:80/",
  });
  const torResponse = await api.get(
    "/api/tx/a84dbcf0d2550f673dda9331eea7cab86b645fd6e12049755c4b47bd238adce9",
    {
      headers: {
  ...
  if (
    json.txid !==
    "a84dbcf0d2550f673dda9331eea7cab86b645fd6e12049755c4b47bd238adce9"
  )
